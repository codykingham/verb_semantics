...
class SemSpace:
    ...
    def __init__(self, experiment, info=True, test=False, run_ll=False, verb_space=True):
        ...
        self.raw_norm = self.raw / self.raw.sum() # normalization for small-scale, categorical variables
        ...
        self.distance_raw_norm = pd.DataFrame(self.pairwise_raw_norm, columns=self.raw.columns, index=self.raw.columns) # NB: no glosses
        ...
        sim_pmi_noDiag = pd.DataFrame(pmi_values, self.sim_pmi_nogloss.index, self.sim_pmi_nogloss.columns)
        self.sim_pmi_maxNorm = sim_pmi_noDiag / sim_pmi_noDiag.max().quantile(q=0.75) # divide by 75th percentile
        ...
        self.sim_rRatio_maxNorm = sim_rRatio_maxNorm / sim_rRatio_maxNorm.max().quantile(q=0.75) # divide by maximum potential similarity   
